...
| `--store-dir` | `SUM_STORE_DIR` | `$HOME/.sumnode/store` | Root for chunks and manifests. Resolution is flag, then env, then `$HOME`; CWD is never consulted. Two nodes sharing a root share one chunk namespace and one `manifests/` directory, silently — set this per node when running more than one on a host. |
