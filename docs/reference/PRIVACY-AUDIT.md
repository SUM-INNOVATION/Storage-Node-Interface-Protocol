...
|-----|---------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------|
| 1   | Logs leak `K_file` plaintext                                                                            | No `info!`/`warn!`/`debug!`/`println!`/`eprintln!` formats `k_file` or `K_file`.   | [`scripts/audit-logs.sh`](../../scripts/audit-logs.sh) — guardrail run on every `make release-check`; allowlist marker is `// audit-allow: <reason>` |
...
| 8   | V2 Private ACL fails open on chain RPC error                                                            | `find_my_access_entry` maps any RPC error during pagination to `PrivateDownloadError::Rpc`, NOT `NoAccess`. Distinct typed variants prevent conflating "not on list" with "could not determine." | `find_my_access_entry_fails_closed_on_rpc_error` test in [`download_private.rs`](../../crates/sum-node/src/download_private.rs) |
| 9   | V1 fallback path silently downloads a Private V2 file                                                   | [`route_download_target`](../../crates/sum-node/src/download_route.rs) routes Public/Private/None deterministically; unknown visibility byte fails closed with `RouteError::UnknownV2Visibility`. Decision depends ONLY on the V2 chain row, not local chunk store / peer state. | 5 routing tests in [`download_route.rs`](../../crates/sum-node/src/download_route.rs) — `route_none_is_v1_legacy`, `route_some_public_is_v2_public`, `route_some_private_is_v2_private`, `route_unknown_visibility_byte_fails_closed`, `route_unknown_visibility_does_not_downgrade_to_public` |
...
| 12  | Temp ciphertext file from Private ingest persists on abort                                              | [`PrivateArtifacts._ciphertext_temp`](../../crates/sum-node/src/ingest_v2.rs) is `tempfile::NamedTempFile`; drop on success/abort cleans up. | `private_artifacts_temp_ciphertext_is_removed_on_drop` test in [`ingest_v2.rs`](../../crates/sum-node/src/ingest_v2.rs) |
...
| 14  | Forward secrecy on revoke (revoked party retains decryptable cached ciphertext)                          | **NOT mitigated.** Documented as Phase 5+. Operators warned in [`OPERATOR-RUNBOOK.md`](../operations/OPERATOR-RUNBOOK.md). | N/A — known gap; revocation does NOT rotate `K_file`; for forward secrecy, revoke + re-ingest under a fresh `K_file`. |
| 15  | Chain returns ambiguous response that admits Private fetch                                              | [`access.rs`](../../crates/sum-node/src/access.rs) treats RPC error as `V2NotEnabled`; production profile fails closed. Pagination errors during access-list resolution surface as typed `Rpc`, never `NoAccess`. | `find_my_access_entry_fails_closed_on_rpc_error` in [`download_private.rs`](../../crates/sum-node/src/download_private.rs) |
...
- Each row checked at every release (see
  [`RELEASE-CHECKLIST.md`](../operations/RELEASE-CHECKLIST.md) §6).
