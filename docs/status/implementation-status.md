...
| Post-#62 `AssignmentCoverageV2` epoch fields (`assignment_epochs`, `latest_assignment_epoch`, `reassignment_needed`, `per_epoch`) | Shipped (observability-only) | [`crates/sum-types/src/rpc_types.rs`](../../crates/sum-types/src/rpc_types.rs) `AssignmentCoverageV2`; `AssignmentEpochCoverageV2`; `AssignmentCoverageV2::resolved_latest_epoch` | All four fields deserialize with `#[serde(default)]` for forward compatibility with legacy chain responses. Aggregate `can_activate_now` semantics are unchanged post-#62 (see upstream `storage_metadata.rs:2181-2200`). `ReassignChunksV2` is NOT implemented in SNIP; the fields are read for observability only. Limitation: `#[serde(default)]` cannot distinguish "field missing" from "field explicitly zero/empty" — malformed partial metadata is silently treated as legacy. |
| Runtime `R = 0` preflight (fresh ingest + resume) | Shipped | [`crates/sum-node/src/ingest_v2.rs`](../../crates/sum-node/src/ingest_v2.rs) `IngestPipeline::run` / `run_private` / `resume`; [`crates/sum-node/src/main.rs`](../../crates/sum-node/src/main.rs) `run_ingest_v2` / `run_resume_v2` | When `chain_getChainParams.assignment_replication_factor == 0`, no assignment can be built for any chunk. Fresh ingest short-circuits BEFORE `RegisterFilePendingV2` — no chain state created, no fees spent. Resume short-circuits BEFORE any S2/S3 push or S5 activation — the on-chain Pending file is unchanged. |
| Post-S1 pinned empty snapshot recovery | Shipped | [`crates/sum-node/src/ingest_v2.rs`](../../crates/sum-node/src/ingest_v2.rs) `IngestPipeline::run` / `run_private` / `resume` | When `storage_getActiveNodesAtHeight(assignment_height)` returns 0 eligible archives after `filter_active_archives`, the pipeline returns `PendingNeedsAction { suggested: Abandon }` with root and manifest preserved. The pinned assignment height is historical and immutable — subsequent `resume` calls will observe the same empty snapshot. External `ReassignChunksV2` is the (chain-side, unsupported here) recovery. |
...
| Feature | Status | Location | Notes |
|---|---|---|---|
| Deterministic chunk assignment (runtime `R`) | Shipped | [`crates/sum-store/src/assignment.rs`](../../crates/sum-store/src/assignment.rs); [`crates/sum-store/src/assignment_v2.rs`](../../crates/sum-store/src/assignment_v2.rs) | `R` is fetched at runtime from `chain_getChainParams.assignment_replication_factor` (`RuntimeChainParams`) and plumbed into every eligibility consumer — upload, MarketSync fetch, GC retained-set, PushValidator, AssignmentAttestor. Effective replication is `min(R, N)` (matches upstream `sum-chain/crates/primitives/src/storage_metadata.rs:299-302`). The dev-fallback constant `DEFAULT_REPLICATION_FACTOR` (formerly `REPLICATION_FACTOR`) is used ONLY in the `NodeProfile::Dev` fallback when the chain-params RPC fails; production hard-fails to avoid a silent divergence. See [`../reference/chain-compat.md`](../reference/chain-compat.md) for the eligibility contract and consumer audit table. |
