...
- [`OPERATOR-RUNBOOK.md`](runbook.md) — runbook with
  per-archive registration, recovery scenarios, monitoring, security
  posture.
- [`RELEASE-CHECKLIST.md`](../release/release-checklist.md) § 7 —
...
[`OPERATOR-RUNBOOK.md`](runbook.md) § "Mainnet bring-up"
...
`--chain-id 1` is required on the current release —
`register-encryption-key` signs with the CLI-supplied `chain_id`
rather than reading it from RPC. The workspace default is `1337`,
which no deployment recognises. See
[`../reference/config-flags.md`](../reference/config-flags.md)
"Chain ID safety" for the full list of affected commands.
...
`register-node` reads `chain_id` live from RPC via
`chain_getChainParams`, so no `--chain-id` override is required
here. Same `tx_hash:` / `finalized_height:` stdout contract.
...
`--chain-id 1` is required on the current release. The `listen`
loop runs a background `AssignmentAttestor` that submits
`AcceptAssignmentV2` transactions on every V2 push wave; the
attestor signs with the CLI-supplied `chain_id`. Without an
explicit `--chain-id 1`, the attestor uses the workspace default
`1337`, and the chain will not admit those transactions — the
archive holds chunks locally but cannot advance them to `Accepted`
in `storage_getAssignmentCoverageV2`, so ingest clients see files
stall in `Pending`. See
[`../reference/config-flags.md`](../reference/config-flags.md)
"Chain ID safety."
...
documented in [`OPERATOR-RUNBOOK.md`](runbook.md)
...
from [`RELEASE-CHECKLIST.md`](../release/release-checklist.md) § 7 to pass:
...
[`OPERATOR-RUNBOOK.md`](runbook.md) "Recovery scenarios"
