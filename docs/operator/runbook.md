...
[`RELEASE-CHECKLIST.md`](../release/release-checklist.md) (release flow).
...
- derives the X25519 encryption keypair via HKDF (domain
  `snip-x25519-encryption-key-v1`) used for Private V2 files.
...
> and L1 address (verified by [`PRIVACY-AUDIT.md`](../security/privacy-audit.md)).
...
> ```
>
> **Set `SUM_CHAIN_ID` explicitly.** Several write commands
> (`register-encryption-key`, `share`, `revoke`, `update-access`,
> `abandon`, and `listen`'s background attestor) sign with the
> CLI-supplied `chain_id`. The workspace default is `1337`, which
> matches no deployment. See
> [`../reference/config-flags.md`](../reference/config-flags.md)
> "Chain ID safety" for the full list.
...
#    On a mainnet or live-chain RPC URL, e2e-helper's write-gate refuses
#    to submit without --allow-live-chain-write. Reads chain_id live from
#    RPC (safe against the workspace default of --chain-id 1337).
...
# 4. Start serving. Pass --chain-id explicitly — the listener's
#    background AssignmentAttestor signs with the CLI value.
cargo run --release -p sum-node -- --chain-id 1 listen
```
...
same seed submits a fresh transaction that overwrites the on-chain
slot with the same value). It validates the operator's end-to-end
signing + finality flow before any stake is committed.
...
`--chain-id 1` is required on the current release. The command
signs with the CLI value; the workspace default is `1337`, which
no deployment recognises. See
[`../reference/config-flags.md`](../reference/config-flags.md)
"Chain ID safety."
...
investigate before any larger write — likely causes are wrong
`chain_id`, insufficient balance, or RPC drift.
...
never use `--profile dev` against mainnet. `--chain-id 1` is
required on the current release — the listener's background
`AssignmentAttestor` signs `AcceptAssignmentV2` with the CLI value
and would otherwise use the workspace default `1337`.
...
[`RELEASE-CHECKLIST.md`](../release/release-checklist.md) §4 and §5). The
...
- Logs do not contain seed, `K_file`, X25519 secret, or plaintext.
  Pinned by [`PRIVACY-AUDIT.md`](../security/privacy-audit.md) and the audit
