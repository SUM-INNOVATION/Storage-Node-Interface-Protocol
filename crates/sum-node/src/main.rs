...
use sum_net::{SumNet, SumNetEvent, Keypair, TOPIC_STORAGE, TOPIC_TEST};
...
#[derive(Subcommand)]
enum Command {
    /// Listen indefinitely, serving chunk requests and printing events.
    Listen,
...
async fn main() -> Result<()> {
    ...
    let keypair = if let Some(ref key_path) = cli.key_file {
        ...
        warn!("no --key-file provided — generating random keypair (dev mode)");
        Keypair::generate_ed25519()
    };

    match cli.command {
        Command::Listen            => run_listen(keypair).await,
...
fn hex_to_bytes_32(hex: &str) -> Result<[u8; 32]> {
    if hex.len() != 64 {
        anyhow::bail!("key file must contain exactly 64 hex characters (32 bytes), got {}", hex.len());
    }
...
async fn run_listen(keypair: Keypair) -> Result<()> {
    let mut net = SumNet::new(NetConfig::default(), keypair).await?;
    let store = SumStore::new(StoreConfig::default())?;

    info!("SUM Node listening — serving chunks, press Ctrl-C to stop");
...
/// After announcing, serve chunk requests until Ctrl-C.
async fn serve_loop(net: &mut SumNet, store: &SumStore) -> Result<()> {
    loop {
        tokio::select! {
            Some(event) = net.next_event() => {
                match &event {
